...
async function main() {
  ...
  for (const p of POOLS) {
    ...
    const pool = await prisma.pool.create({
      data: {
        ...
        stage: p.stage, trending: !!p.trending, isNew: !!p.isNew, verified: true, feeTier: 0.3,
      },
    });
    // seed a few activity rows per pool
    const n = 3;
    for (let i = 0; i < n; i++) {
  ...
  console.log(`Seeded ${pools} pools and ${acts} activity rows.`);
}
